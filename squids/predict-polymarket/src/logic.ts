...
// operating on the cache interface. Entities are mutable TypeORM class instances,
...
export async function getGlobal(cache: IEntityCache): Promise<Global> {
...
export async function processTradeActivity(
  ...
  if (participant == null) {
    if (question == null) {
      // FK-constrained (unlike graph-node/Envio): the participant is created
      // without a question link and can never settle. Expected for markets
      // whose metadata was rejected; anything else deserves investigation.
      cache.log.warn(
        `MarketParticipant ${participantId} created without Question ${conditionId}`,
      );
    }
