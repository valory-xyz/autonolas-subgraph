...
| `src/rpc.ts` | Safe getOwners/getThreshold, memoized; pinned to the sighting block |
...
whole history to the wrong place. During backfill the sighting block is
millions of blocks old, so a pruned node cannot serve it; the processor
