...
export function handleRequest(event: RequestEvent): void {
  ...
  if (serviceId !== null) {
    ...
    if (mechEntity !== null) {
      ...
      log.warning('handleRequest: Could not find Mech entity for serviceId {} (from mech {})', [
        serviceId.toString(),
        event.params.mech.toHexString()
      ]);
    }
  } else {
    log.warning('handleRequest: getServiceIdFromMech returned null for mech {}', [event.params.mech.toHexString()]);
  }
...
export function handleRevokeRequest(event: RevokeRequestEvent): void {
  ...
  // If the request was already marked delivered, ignore revoke
