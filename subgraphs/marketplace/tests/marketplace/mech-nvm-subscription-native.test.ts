...
import { handleDeliver, handleRequest, handleMaxDeliveryRateUpdated } from "../../src/marketplace/mech-nvm-subscription-native"
import { handleMarketplaceRequest } from "../../src/marketplace/mech-marketplace"
import { createDeliverEvent, createRequestEvent, createMaxDeliveryRateUpdatedEvent } from "./mech-nvm-subscription-native-utils"
...
describe("Mech NVM Subscription Native Handler", () => {
  ...
  test("Other-mech delivery increments deliveredByOthersFromReceived counter", () => {
    ...
    handleMarketplaceRequest(marketplaceRequestEvent)
    handleRequest(createRequestEvent(priorityMech, TEST_REQUEST_ID_1, TEST_DATA_NATIVE))
    handleDeliver(createDeliverEvent(deliveryMech, TEST_REQUEST_ID_1, TEST_MECH_SERVICE_MULTISIG, deliveryRate, TEST_DATA_NATIVE))
  ...
  test("Multiple self-deliveries increment counter correctly", () => {
    ...
    // Simulate marketplace flow
    let marketplaceRequestEvent1 = createMarketplaceRequestEvent(testMech, TEST_REQUESTER, [requestId1], [TEST_DATA_NATIVE])
    handleMarketplaceRequest(marketplaceRequestEvent1)
    handleRequest(createRequestEvent(testMech, requestId1, TEST_DATA_NATIVE))

    let marketplaceRequestEvent2 = createMarketplaceRequestEvent(testMech, TEST_REQUESTER, [requestId2], [TEST_DATA_NATIVE])
