...
export function getEthUsd(block: ethereum.Block): BigDecimal {
  // No Chainlink feeds available on Mode network
  // Use WETH/USDC Velodrome pool pricing directly
  log.info("[ETH_PRICE] Using WETH/USDC Velodrome pool pricing", [])
  return getUsd(WETH, block) // Use WETH address from constants
}
