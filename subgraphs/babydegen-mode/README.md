...
- **Only native USDC and ETH count as funding.** `netUsd` increases on inbound native-USDC ERC20 transfers and inbound ETH (`SafeReceived`) whose sender passes the funding-source test: the service's operator safe, or any EOA. Bridged USDC (USDC.e) is explicitly excluded from funding.
